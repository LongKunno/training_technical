...
func NewRouter(engine *papertrading.Engine) http.Handler {
	...
	mux.HandleFunc("/api/paper/sessions", handlers.NewPaperSessionsHandler(engine))
	...
	mux.HandleFunc("/api/paper/timeline", handlers.NewPaperTimelineHandler(engine))
	mux.HandleFunc("/api/paper/timeline/stream", handlers.NewPaperTimelineStreamHandler(engine))
