...
LOAD_POD_NAME="${LOAD_POD_NAME:-data-pipeline-memory-load}"
WINDOW_SECONDS="${WINDOW_SECONDS:-90}"
PARALLEL_REQUESTS="${PARALLEL_REQUESTS:-2}"
REQUEST_DURATION_MS="${REQUEST_DURATION_MS:-20000}"
ALLOCATION_MIB="${ALLOCATION_MIB:-96}"
...
kubectl -n "${K8S_NAMESPACE}" run "${LOAD_POD_NAME}" \
  ...
  --env="TARGET_URL=http://${service_name}.${K8S_NAMESPACE}.svc.cluster.local:8000/internal/ops/benchmark/memory" \
  ...
  --env="ALLOCATION_MIB=${ALLOCATION_MIB}" \
  --command -- sh -ec '
      ...
      (
        while [ "$(date +%s)" -lt "$end" ]; do
          curl -fsS -X POST "$TARGET_URL" \
            ...
            -d "{\"duration_ms\":${REQUEST_DURATION_MS},\"allocation_mib\":${ALLOCATION_MIB}}" >/dev/null
        done
      ) &
      worker=$((worker + 1))
    done
